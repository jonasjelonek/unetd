...
network_host_create(struct network *net, struct blob_attr *attr, bool dynamic)
{
	...
	pqc_key_buf = blobmsg_get_string(tb[NETWORK_HOST_PQC_KEY]);
	if (pqc_key_buf) {
        strsep(&pqc_key_buf, ":");

        if (pqc_key_buf) {
            has_pqc_key = (b64_decode(pqc_key_buf, pqc_key, SNTRUP761_PUB_SIZE) == SNTRUP761_PUB_SIZE);
        }
	}
	...
	if (has_pqc_key) {
	    memcpy(peer->pqc_key, pqc_key, sizeof(pqc_key));
